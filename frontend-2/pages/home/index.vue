...
const splideMoved = (splide, destIndex) => {
  console.log(destIndex);
  const selectedElement = document.getElementById(`slide-${destIndex}`);
  bookExRemoveNone(selectedElement, destIndex);
};

const bookExRemoveNone = (selectedElement, Index) => {
  console.log("bookExRemoveNone");
  console.log(Index);
  ...
  document
    ...
    .forEach((item) => {
      if (item.classList.contains(`textbox-recent-tree-info__frame-${Index}`)) {
  ...
  document.querySelectorAll(".book-tree-explanation-item").forEach((item) => {
    if (item.classList.contains(`book-tree-explanation-item-${Index}`)) {
      item.classList.remove("none");
    } else {
      item.classList.add("none");
    }
...
//-------------------LifeCycle-------------------
onMounted(() => {
  //   const carouselBox = document.querySelector(".tree-book-bg");
  ...
  //   const compWidth = ref(
  //     parseFloat(window.getComputedStyle(carouselBox).width.replace("px", ""))
  //   );
  //   console.log(compWidth.value);
  //   const width = compWidth.value * 0.4;
  //   const midWidth = compWidth.value * 0.2;
  //   // CSS 변수 설정
    ...
    <section class="my-tree-container">
      ...
      <section class="book-tree-info-container">
        ...
        <section class="book-img-container">
          ...
          >
            <SplideSlide v-for="(tree, index) in treeInfoList">
              <NuxtLink :to="`/tree/${tree.treeId}`">
                <img
                  class="mid__img"
        ...
        <section class="book-tree-explanation-container">
          ...
          <ul class="book-tree-explanation-list">
            <li
              v-for="(tree, index) in treeInfoList"
              :class="`book-tree-explanation-item-${index} none`"
              :key="index"
            >
